...
set -gx AST_GREP_CONFIG $HOME/.config/ast-grep/sgconfig.yml


# Note, this is for a local instance of obsidian
# set OBSIDIAN_REST_API_KEY in ~/.config/fish/local.fish (not version controlled)
...
for p in $HOME/bin $HOME/.krew/bin $HOME/.local/bin $HOME/.kubescape/bin $HOME/.toolhive/bin
...
if status is-interactive
    ...
    # Auto-start herdr in Ghostty only (after mise so the correct version is used)
    if set -q GHOSTTY_RESOURCES_DIR; and not set -q HERDR_ENV; and command -q herdr
        exec herdr
    end
...
# strix
fish_add_path $HOME/.strix/bin

# sparkrun tab-completion
# Cache to an autoloaded completions file instead of regenerating on every
# shell (the live probe cost ~385ms of Python cold-start per startup).
# Fish autoloads completions/sparkrun.fish on demand; regenerate only when the
# binary is newer than the cached file.
set -l spark_comp ~/.config/fish/completions/sparkrun.fish
if command -q sparkrun; and test (command -v sparkrun) -nt $spark_comp
    _SPARKRUN_COMPLETE=fish_source sparkrun >$spark_comp 2>/dev/null
end

...
LINTERS: dict[str, LintFunc] = {
    ...
    ".yaml": lint_yaml,
    ".yml": lint_yaml,
    # ".md": lint_markdown,  # temporarily disabled
    ".tf": lint_terraform,
}
...
def get_handlers(
    ...
    # Check GitHub Actions workflows (only .yml/.yaml, not gh-aw .md source files)
    if GITHUB_ACTIONS_PATTERN in file_path and ext in (".yml", ".yaml"):
...
def main() -> None:
    ...
    if reformatted_files:
        reformat_notice = (
            ...
            "If you need to make further edits to these files, re-read them first, then continue with your original task:\n"
        )
    ...
    if lint_output:
        lint_notice = (
            ...
            "Fix ALL of these, then continue with your original task.\n\n"

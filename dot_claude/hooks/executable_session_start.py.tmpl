...
def get_beads_context() -> str:
    """Return formatted beads context (active + ready work), or empty string on failure."""
    try:
        in_progress = subprocess.run(
            ["bd", "list", "--status=in_progress", "--flat", "-n", "10"],
            capture_output=True,
            text=True,
            check=False,
            timeout=8,
    ...
    in_progress_out = in_progress.stdout.strip() if in_progress.returncode == 0 else ""
    ready_out = ready.stdout.strip() if ready.returncode == 0 else ""

    if in_progress_out:
        lines.append("Active beads (in-progress):")
        for line in in_progress_out.splitlines():
            if line.strip():
    ...
    if ready_out:
        if lines:
            lines.append("")
        lines.append("Next available beads (ready):")
...
def get_recent_memory() -> str:
    ...
    Talks to the long-running basic-memory MCP service via ~/.claude/bin/bm-rpc
    instead of the `basic-memory` CLI. The CLI eagerly imports the full
    FastAPI app on every invocation (~11s cold start); bm-rpc hits the
    already-running HTTP service in ~300-700ms.
    """
    bm_rpc = Path.home() / ".claude" / "bin" / "bm-rpc"
    if not bm_rpc.exists():
        return ""
    ...
    try:
        result = subprocess.run(
            [
                ...
                "call",
                "recent_activity",
                '{"timeframe":"7d","type":["entity"],"page_size":5}',
            ],
            capture_output=True,
            text=True,
            check=False,
        ...
        text = (
            payload.get("result", {}).get("content", [{}])[0].get("text", "")
        ).strip()
        if not text or text.startswith("Error"):
            return ""
        # The MCP tool returns a pre-formatted markdown block; strip the
        # header so we can slot it under our own "Recent Basic Memory"
        # section without duplicating a title.
        lines = [
            line
            for line in text.splitlines()
            if line.strip() and not line.startswith("## Recent Activity:")
        ]
        return "\n".join(lines)
...
def main() -> None:
    ...
    beads_context = get_beads_context()
    beads_section = f"\n\n{beads_context}" if beads_context else ""

    context_message = f"""Project: {project}{git_line}
{agents_section}{memory_section}{beads_section}{test_tip}"""

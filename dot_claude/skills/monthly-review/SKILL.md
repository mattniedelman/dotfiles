...
```python
recent_activity(timeframe="30 days")  # Basic Memory MCP

search_notes(query="completed done finished shipped")  # Basic Memory MCP

search_notes(query="decision decided")  # Basic Memory MCP

search_notes(query="blocked issue problem challenge")  # Basic Memory MCP
```

### 3. Check for Quarterly Goals

```python
search_notes(query="quarterly goals tactical goals strategic goals")  # Basic Memory MCP
```

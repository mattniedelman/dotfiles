...
1. Read plan, extract all tasks with full text, note context.
   Create a beads issue per task (`bd create --title="..." --type=task`)
2. For each task:
   ...
   - Run `bd close <id>` to mark task complete
3. After all tasks:
...
- **executing-plans** - Use for parallel session instead of same-session
  execution

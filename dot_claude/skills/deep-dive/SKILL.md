...
**CRITICAL:** All deep dive output MUST be saved to Basic Memory using the
`write_note` tool (Basic Memory MCP).
NEVER write files to the local filesystem for this purpose.
...
Glob(pattern="relevant/directory/**")

# Find main symbols (Serena MCP)
...
```python
# Search existing knowledge (Basic Memory MCP)
search_notes(query="topic keywords")

# External search
WebSearch(query="[topic] overview introduction")
```
...
```python
# Read the implementation (Serena MCP)
find_symbol(name_path="critical_function", include_body=True)

# Follow the call chain (Serena MCP)
...
```python
# Find error handling
Grep(pattern="raise|except|Error", path="file.py")

# Find TODOs and FIXMEs
Grep(pattern="TODO|FIXME|HACK|XXX", path="file.py")

# Find validation logic
Grep(pattern="if.*not|assert|validate", path="file.py")
```
...
Use `write_note` (Basic Memory MCP).

...
| Web UI | `uvx rodney` or Playwright |
...
### Web UIs (Browser Automation)

```bash
# Using Rodney (lightweight Chrome automation)
uvx rodney --help # Learn commands

# Screenshot a page
uvx rodney screenshot http://localhost:3000/dashboard

# Click and interact
uvx rodney click "button#submit"
uvx rodney type "input#search" "test query"
uvx rodney screenshot /tmp/after-search.png

# Read accessibility tree
uvx rodney a11y http://localhost:3000/form
```
...
## Documentation with Showboat

Capture manual testing session for evidence:

```bash
# Create session document
uvx showboat init manual-test.md

# Record commands and their output
uvx showboat exec "curl http://localhost:8000/health"
uvx showboat note "Health check returns expected status"

uvx showboat exec "curl http://localhost:8000/api/users | jq '.count'"
uvx showboat note "User count matches expected test data"
```

...
**Rule**:
When working in any git repository, treat it as a project and apply specs-based
development.
This workflow ensures implementation aligns with requirements and is inspired by
the Ralph Wiggum technique, adapted for interactive development.
...
- The user requests spec-related work ("create spec", "plan", "gap analysis",
  etc.)

**Session Start Behavior**:
When entering a git repository, check for:
...
**CRITICAL**:
Before implementing ANY functionality, search the codebase to confirm it doesn't
already exist.
This is the most common failure mode - duplicating or conflicting with existing
code.
...
Can you describe the topic in one sentence without "and" conjoining unrelated
capabilities?
...
**AI Role**:
Help clarify requirements, identify edge cases, suggest acceptance criteria.
Ask probing questions before writing specs.
...
**AI Role**:
Perform gap analysis, identify what's missing, prioritize tasks.
...
5. Update plan:
   mark complete, note discoveries
6. Commit when tests pass

**AI Role**:
Implement one task at a time, validate against acceptance criteria, update plan.
...
If backpressure fails, fix before proceeding.
Do not mark tasks complete until validation passes.

### When Validation Method Is Unclear

**Rule**:
If the appropriate validation method is not obvious, ASK before proceeding.
...
Options:
[suggest relevant options based on context]"
...
3. **Specs are source of truth** - Implementation must satisfy acceptance
   criteria
...
**When in doubt**:
Ask "Should I create a spec for this, or proceed directly?"
